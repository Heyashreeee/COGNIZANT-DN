-- 🧹 STEP 0: Clean up old tables if needed
...
-- 🔍 INDEX EXERCISE 1: Non-clustered index on ProductName
-- Before index: full scan
SELECT * FROM Products WHERE ProductName = 'Laptop';

-- Create index to speed up ProductName search
CREATE NONCLUSTERED INDEX idx_ProductName
ON Products(ProductName);

-- After index: faster search
SELECT * FROM Products WHERE ProductName = 'Laptop';

-- 📅 INDEX EXERCISE 2: Non-clustered index on OrderDate
-- (Orders already has a clustered PK on OrderID)
...
-- 👥 INDEX EXERCISE 3: Composite index on CustomerID + OrderDate
